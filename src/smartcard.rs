...
impl SmartCardSlotManager {
    ...
    pub fn default_manager() -> Option<Self> {
        let raw = unsafe { ffi::ctk_slot_manager_default() };
        (!raw.is_null()).then_some(Self { raw })
    }

    pub fn slot_names(&self) -> Result<Vec<String>, CryptoTokenKitError> {
        let mut error_ptr = ptr::null_mut();
        let json = unsafe { ffi::ctk_slot_manager_slot_names_json(self.raw, &mut error_ptr) };
    ...
    fn get_slot_impl(
        ...
        let status = unsafe {
            if use_reply_api {
                ffi::ctk_slot_manager_get_slot_with_name(
                    self.raw,
                    name.as_ptr(),
                    &mut raw,
                    &mut error_ptr,
                ...
                ffi::ctk_slot_manager_slot_named(self.raw, name.as_ptr(), &mut raw, &mut error_ptr)
            }
        };
...
impl SmartCardSlot {
    pub fn name(&self) -> Result<String, CryptoTokenKitError> {
        let ptr = unsafe { ffi::ctk_slot_name(self.raw) };
        if ptr.is_null() {
    ...
    pub fn max_input_length(&self) -> isize {
        unsafe { ffi::ctk_slot_max_input_length(self.raw) }
    }

    #[must_use]
    pub fn max_output_length(&self) -> isize {
        unsafe { ffi::ctk_slot_max_output_length(self.raw) }
    }

    #[must_use]
    pub fn state(&self) -> SlotState {
        SlotState::from_raw(unsafe { ffi::ctk_slot_state(self.raw) })
    }

    #[must_use]
    pub fn make_smart_card(&self) -> Option<SmartCard> {
        let raw = unsafe { ffi::ctk_slot_make_smart_card(self.raw) };
        (!raw.is_null()).then_some(SmartCard { raw })
    }
...
impl SmartCard {
    pub fn send_ins(
        ...
        let (data_ptr, data_len) =
            data.map_or((ptr::null(), 0), |data| (data.as_ptr(), data.len()));
        let mut reply_ptr = ptr::null_mut();
        let mut error_ptr = ptr::null_mut();
        let status = unsafe {
            ffi::ctk_smart_card_send_ins(
                self.raw,
                ins,
                p1,
                p2,
