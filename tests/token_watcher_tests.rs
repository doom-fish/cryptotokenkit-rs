...
fn watcher_lists_tokens_and_triggers_callbacks() -> Result<(), Box<dyn std::error::Error>> {
    ...
    watcher.add_removal_handler("missing-token", move |token_id| removed_clone.lock().unwrap().push(token_id))?;

    let token_ids = watcher.token_ids()?;
    assert!(!token_ids.is_empty());
    assert!(!inserted.lock().unwrap().is_empty());
    assert_eq!(removed.lock().unwrap().as_slice(), &[String::from("missing-token")]);
